...
df = pd.read_csv(fn)
df = df[df.municipality != 'municipality'] #filter out rows with header as vlaues(error in appendign)

coords = [WGS84Point(literal_eval(coord)[0], literal_eval(coord)[1]) for coord in df.center_coord.unique()]
print(len(coords))
